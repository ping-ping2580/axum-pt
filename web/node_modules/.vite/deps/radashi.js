//#region node_modules/.pnpm/radashi@12.5.1/node_modules/radashi/dist/radashi.js
...
function cartesianProduct(...arrays) {
	...
	for (const array of arrays) {
		const result = [];
		for (const currentArray of out) for (const item of array) {
...
function cluster(array, size = 2) {
	const clusters = [];
	if (size > 0) for (let i = 0; i < array.length; i += size) clusters.push(array.slice(i, i + size));
	return clusters;
}
function concat(...values) {
	const result = [];
	const append = (value) => value != null && result.push(value);
	for (const value of values) if (Array.isArray(value)) value.forEach(append);
	else append(value);
	return result;
}
...
function fork(array, condition) {
	const forked = [[], []];
	if (array) for (const item of array) forked[condition(item) ? 0 : 1].push(item);
	return forked;
}
...
function mapify(array, getKey, getValue = (item) => item) {
	...
	for (const [index, item] of array.entries()) map2.set(getKey(item, index), getValue(item, index));
	return map2;
}
function merge(prev, array, toKey) {
	if (!array && !prev) return [];
	if (!array) return [...prev];
	if (!prev) return [];
	if (!toKey) return [...prev];
	const keys2 = /* @__PURE__ */ new Map();
	for (const item of array) keys2.set(toKey(item), item);
...
function pluck(array, mappings) {
	return array.map(mappings ? (item) => mappings.map((mapping) => isFunction(mapping) ? mapping(item) : item[mapping]) : Object.values);
}
...
function replace(array, newItem, match) {
	if (!array) return [];
	if (newItem === void 0) return [...array];
	const out = array.slice();
	for (let index = 0; index < array.length; index++) if (match(array[index], index)) {
		out[index] = newItem;
		break;
	}
	return out;
...
function toggle(array, item, toKey, options) {
	if (!array) return item !== void 0 ? [item] : [];
	if (item === void 0) return [...array];
	let matcher;
	if (toKey) {
		const key = toKey(item, -1);
		matcher = (x, idx) => toKey(x, idx) === key;
	} else matcher = (x) => x === item;
	const existing = array.find(matcher);
	if (existing !== void 0) return array.filter((x, idx) => !matcher(x, idx));
...
function unzip(arrays) {
	...
	const out = new Array(arrays.reduce((max2, arr) => Math.max(max2, arr.length), 0));
	let index = 0;
	const get2 = (array) => array[index];
	for (; index < out.length; index++) out[index] = Array.from(arrays, get2);
	return out;
}
function zip(...arrays) {
	return unzip(arrays);
...
async function all(input) {
	...
	let output;
	if (isArray(input)) output = await Promise.all(input.map((value) => Promise.resolve(value).catch(onError)));
	else {
		output = { ...input };
	...
	if (errors.length > 0) throw new AggregateErrorOrPolyfill(errors);
	return output;
}
...
async function parallel(options, array, func) {
	...
	let signal;
	if (isNumber(options)) options = { limit: options };
	else {
	...
	const queues = Promise.all(list(1, clamp(options.limit, 1, array.length)).map(() => new Promise(processor)));
	let signalPromise;
	if (signal) signalPromise = new Promise((_, reject) => {
	...
	const [errors, results] = fork(sort(flat(itemResults), (r) => r.index), (x) => !!x.error);
	if (errors.length > 0) throw new AggregateErrorOrPolyfill(errors.map((error) => error.error));
...
async function reduce(array, reducer, initialValue) {
	...
	if (acc === void 0 && arguments.length < 3) {
		if (!array.length) throw new TypeError("Reduce of empty array with no initial value");
		acc = array[index++];
	}
	while (index < array.length) acc = await reducer(acc, array[index], index++);
	return acc;
}
...
function timeout(ms, error) {
	return new Promise((_, reject) => setTimeout(() => reject(isFunction(error) ? error() : new TimeoutError(error)), ms));
}
async function toResult(promise) {
	try {
		...
		if (isError(error)) return [error, void 0];
		throw error;
	}
}
function tryit(func) {
...
function memoize(cache, func, keyFunc, ttl) {
	return function callWithMemo(...args) {
		...
		if (existing !== void 0) {
			...
			if (existing.exp > (/* @__PURE__ */ new Date()).getTime()) return existing.value;
		}
		const result = func(...args);
		cache[key] = {
...
function memoLastCall(fn) {
	...
	let lastResult = null;
	return (...args) => {
		if (lastArgs && lastArgs.length === args.length && lastArgs.every((arg, i) => Object.is(arg, args[i]))) return lastResult;
...
var once = /* @__PURE__ */ (() => {
	...
	const once2 = (fn) => {
		const onceFn = function(...args) {
			if (onceFn[onceSymbol] === onceSymbol) onceFn[onceSymbol] = fn.apply(this, args);
...
function proxied(handler) {
	return new Proxy({}, { get: (target, propertyName) => handler(propertyName) });
}
function throttle({ interval, trailing }, func) {
	...
	const trigger = throttled.trigger = (...args) => {
		...
		if (trailing) {
			trailingArgs = void 0;
			clearTimeout(timer);
			timer = setTimeout(() => trailingArgs && trigger(...trailingArgs), interval);
		}
	};
	return throttled;
}
...
function castComparator(mapping, compare, reverse) {
	...
	const comparator = (left, right) => {
		...
		if (compare) return compare(mappedLeft, mappedRight);
...
function clamp(n, min2, max2) {
	if (max2 != null && min2 != null && min2 > max2) throw new Error("invalid clamp range");
...
function* range(startOrLength, end, valueOrMapper = (i) => i, step = 1) {
	...
	for (let i = start; i <= final; i += step) yield mapper(i);
}
...
function toFloat(value, defaultValue) {
	const parsedValue = isSymbol(value) ? NaN : Number.parseFloat(value);
	return Number.isNaN(parsedValue) ? defaultValue !== void 0 ? defaultValue : 0 : parsedValue;
}
function toInt(value, defaultValue) {
	const parsedValue = isSymbol(value) ? NaN : Number.parseInt(value);
	return Number.isNaN(parsedValue) ? defaultValue !== void 0 ? defaultValue : 0 : parsedValue;
}
...
function clone(obj) {
	...
	for (const key of Object.getOwnPropertyNames(obj)) newObj[key] = obj[key];
	return newObj;
}
var DefaultCloningStrategy = {
	cloneMap(input, track, clone2) {
		const output = track(/* @__PURE__ */ new Map());
		for (const [key, value] of input) output.set(key, clone2(value));
		return output;
	},
	cloneSet(input, track, clone2) {
		const output = track(/* @__PURE__ */ new Set());
		for (const value of input) output.add(clone2(value));
		return output;
	},
	...
	cloneObject(input, track, clone2) {
		...
		for (const key of Reflect.ownKeys(input)) {
			...
			if ("value" in descriptor) descriptor.value = clone2(descriptor.value);
...
var FastCloningStrategy = { cloneObject: (input, track, clone2) => {
	const output = track({ ...input });
	for (const key of Object.keys(input)) output[key] = clone2(input[key]);
	return output;
} };
function cloneDeep(root, customStrategy) {
	...
	const cloneDeep2 = (parent, strategy2) => {
		...
		if (!newParent) return cloneDeep2(parent, DefaultCloningStrategy);
		tracked.set(parent, newParent);
		return newParent;
	};
...
function crush(value) {
	...
	return function crushReducer(crushed, value2, path) {
		if (isObject(value2) || isArray(value2)) for (const [prop, propValue] of Object.entries(value2)) crushReducer(crushed, propValue, path ? `${path}.${prop}` : prop);
		else crushed[path] = value2;
		return crushed;
	}({}, value, "");
}
...
function keys(value) {
	...
	const recurse = (value2) => {
		if (isPlainObject(value2)) for (const [prop, propValue] of Object.entries(value2)) {
			keyPath.push(prop);
			recurse(propValue);
			keyPath.pop();
		}
		else if (isArray(value2)) value2.forEach((item, index) => {
			keyPath.push(index);
			recurse(item);
			keyPath.pop();
		});
		else keys2.push(keyPath.join("."));
	};
	recurse(value);
	return keys2;
}
...
function pick(obj, filter) {
	if (!obj) return {};
	let keys2 = filter;
	if (isArray(filter)) filter = null;
	else keys2 = Reflect.ownKeys(obj);
	return keys2.reduce((acc, key) => {
		if (filterKey(obj, key, filter)) acc[key] = obj[key];
		return acc;
	}, {});
}
function set(initial, path, value) {
	...
	return function recurse(object, keys2, index) {
		...
		if (isDangerousKey(key, object)) throw new Error("Unsafe key in path: " + key);
		if (index < keys2.length - 1) value = recurse(object[key], keys2, index + 1);
...
function traverse(root, visitor, options, outerContext) {
	...
	const visit = (value, key) => {
		if (context.parent.constructor === Map) [key, value] = value;
		context.path.push(key);
		const result = visitor(context.value = value, context.key = key, context.parent, context, nestedOptions);
		if (result === false) return ok = false;
		if (value !== null && typeof value === "object" && (isArray(value) || isPlainObject(value)) && !context.skipped.has(value) && !context.parents.includes(value)) traverse2(value, result);
		context.path.pop();
		return ok;
	};
	const traverse2 = (parent, parentResult) => {
		...
		if (rootNeedsVisit && parent === root) {
			parentResult = visitor(context.value = parent, context.key = null, context.parent, context, nestedOptions);
			if (parentResult === false) return ok;
		}
		if (isArray(parent)) parent.slice().forEach((value, index, values) => {
			if (visit(value, index) === false) values.length = 0;
		});
		else if (parent === root && isIterable(parent)) {
			let index = 0;
			for (const value of parent) {
				if (visit(value, index) === false) return ok;
				index++;
			}
		} else for (const key of ownKeys(parent)) if (visit(parent[key], key) === false) return ok;
		context.parents.pop();
		context.parent = last(context.parents);
		if (ok && isFunction(parentResult)) ok = parentResult() !== false;
		return ok;
	};
	if (outerContext) {
		if (outerContext.skipped.has(root)) return true;
...
function dedent(text, ...values) {
	var _a;
	if (isArray(text)) {
		if (values.length > 0) return dedent(text.reduce((acc, input, i) => {
			var _a2;
			let value = String(values[i] ?? "");
			const indent2 = value.includes("\n") && ((_a2 = input.match(/[ \t]*(?=[^\n]*$)/)) == null ? void 0 : _a2[0]);
			if (indent2) value = value.replace(/\n(?=[^\n]*?\S)/g, "\n" + indent2);
...
function pascal(str) {
	if (!str) return "";
	const result = str.replace(/(?:[^\w\d]|_|\s)+(\w)([A-Z]+)?/g, (_, firstCharacter, capitalizedLetters) => {
...
function similarity(str1, str2) {
	...
	for (let x = 1; x < numColumns; x++) distances[x] = x;
	for (let y = 1; y < numRows; y++) distances[y * numColumns] = y;
	for (let x = 1; x < numColumns; x++) for (let y = 1; y < numRows; y++) {
		const i = y * numColumns + x;
		distances[i] = Math.min(distances[i - numColumns] + 1, distances[i - 1] + 1, distances[i - numColumns - 1] + (str1[start + y - 1] === str2[start + x - 1] ? 0 : 1));
	}
...
var asyncIteratorSymbol = Symbol.asyncIterator || /* @__PURE__ */ Symbol.for("Symbol.asyncIterator");
...
function isEmpty(value) {
	if (typeof value !== "object" || value === null) return !value || value === true;
